    ...
    return (
        <Dialog.Root open={open} onOpenChange={onOpenChange} {...dialogProps}>
            <Dialog.Portal>
                ...
                <Dialog.Content
                    className={cn("bg-slate-900/90 backdrop-blur-2xl fixed left-1/2 top-1/2 max-h-[85vh] w-full max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-xl p-[25px] border border-white/10 shadow-(--shadow-6) focus:outline-none data-[state=open]:animate-contentShow", className)}>
                    ...
                    <Dialog.Close asChild
                                  ...
                                  aria-label="Close">
                        <Button className={'bg-none bg-transparent border-none'} hoverScale={false}>✕</Button>
                    </Dialog.Close>
                    {closeText &&
                        <Dialog.Close asChild
